...
import { RepositoryIcon } from "hugeicons-react";
...
export default function Report() {
  ...
  const summary = data?.summary ?? { total: 0, completed: 0, pending: 0, cancelled: 0 };
  ...
  return (
    <div className="bg-gray-100 min-h-full">
      <Header
        ...
        description={"Query activity update history with custom date ranges and filters"}
        options={
          <ReportFilter
            filters={filters}
      ...
      <main className="pageWidth pt-6">
        ...
        {submitted && (
          <div className="grid grid-cols-4 gap-4 mb-6">
            {[
              { label: "Total Logs",  value: summary.total,     color: "text-gray-800"  },
        ...
        {submitted && (
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            ...
            {isLoading ? (
              <div className="p-8 text-center text-gray-400 text-sm">Running report...</div>
            ) : logs.length === 0 ? (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100">
                      {["Date", "Activity", "Updated By", "Department", "Action", "Change", "Remarks", "Time"].map((h) => (
                  ...
                  <tbody>
                    {logs.map((log, i) => (
                      <tr key={log.id} className={`border-b border-gray-50 hover:bg-gray-50 transition-colors ${i % 2 ? "bg-gray-50/50" : ""}`}>
                        <td className="px-4 py-3 text-gray-500 font-mono text-xs whitespace-nowrap">
                          {format(new Date(log.log_date), "MMM d, yyyy")}
                        </td>
                        <td className="px-4 py-3 max-w-[180px]">
                          <p className="font-medium text-gray-800 truncate">{log.activity?.title}</p>
                        </td>
                        <td className="px-4 py-3 text-gray-700 whitespace-nowrap">{log.user?.name}</td>
                        <td className="px-4 py-3 text-gray-400 text-xs whitespace-nowrap">{log.user?.department}</td>
                        <td className="px-4 py-3 whitespace-nowrap">
                          <span className={`text-xs font-medium px-2 py-0.5 rounded-full capitalize ${ACTION_BADGE[log.action_type]}`}>
                            {log.action_type}
                          </span>
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap">
                          <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${STATUS_BADGE[log.old_status]}`}>
                            {log.old_status}
                          </span>
                          <span className="text-gray-300 mx-1">→</span>
                          <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${STATUS_BADGE[log.new_status]}`}>
                            {log.new_status}
                          </span>
                        </td>
